...
class DoubleLinkedList:
    ...
    def remove(self, index):
        ...
        # all other cases
        temp = self.get(index)  # before is the node before the index

        temp.next.prev = temp.prev  # break connection to the node after the index
        temp.prev.next = temp.next  # break connection to the node before the index
        temp.next = None  # break connection to the node after the index
        temp.prev = None  # break connection to the node before the index
...
if __name__ == "__main__":
    ...
    print(f"Node deleted: {my_dll.remove(1).value}")
    print("\nModified dll")
    my_dll.print_list()

    print(f"\n--> Remove node at index 3")
    print(f"Node deleted: {my_dll.remove(3).value}")
    print("\nModified dll")
    my_dll.print_list()